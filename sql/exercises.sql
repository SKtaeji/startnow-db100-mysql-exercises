...
SELECT
    ...
    amount,
    payment_date
FROM payment WHERE payment_date >= '2005-05-27 00:00:00';
...
SELECT
	*
FROM customer WHERE last_name like '%m' OR active = 0;
...
SELECT
	*
FROM category WHERE category_id > 4 AND name like 'C%' OR name like 'S%' OR name like 'T%';
...
SELECT
    ...
    payment_date,
    last_update
FROM payment WHERE DATE(payment_date) BETWEEN '2005-05-25 00:00:00' AND '2005-05-25 23:59:59';
...
SELECT
	payment_date,
    amount
FROM payment ORDER BY amount DESC LIMIT 20;
...
SELECT
    ...
    length,
    rental_duration
FROM film WHERE special_features LIKE '%Behind The Scenes%' AND rental_duration BETWEEN '5' AND '7' ORDER BY length DESC LIMIT 10;
...
# 9f. Select the first_name, last_name, address, address2, city name, district, and postal code columns from the staff table, performing 2 left joins with the address table 
...
SELECT
    ...
    address.postal_code
FROM staff 
LEFT JOIN address ON staff.address_id=address.address_id LEFT JOIN city ON address.city_id=city.city_id;
